...
export default function Home() {
  ...
  const [calendarStartFilter, setCalendarStartFilter] =
    useState("00:00");
  const [calendarEndFilter, setCalendarEndFilter] =
    useState("24:00");
  const calendarCourts = courts
  ...
  const getCalendarSlotStatus = (
    ...
    const slotStartMinutes =
      convertTimeToMinutes(slotStart);

    const slotEndMinutes =
      slotEnd === "24:00"
        ? 24 * 60
        : convertTimeToMinutes(slotEnd);
    ...
    for (const booking of calendarBookings) {
      const bookedCourts = booking.court
        .split(",")
        .map((bookedCourt: string) =>
          bookedCourt.trim().toLowerCase()
        );

      const courtIsBooked = bookedCourts.includes(
      ...
      let bookingEnd = convertTimeToMinutes(
        booking.endTime
      );
      ...
      const overlaps =
        bookingStart < slotEndMinutes &&
        bookingEnd > slotStartMinutes;
...
const filteredCalendarCourts = calendarCourts.filter((court) =>
  ...
  const getBookingDateTimes = (
    bookingDate: string,
    bookingStart: string,
    bookingEnd: string
  ) => {
    const start = new Date(bookingDate);
    start.setHours(timeToHour(bookingStart), 0, 0, 0);

    const end = new Date(bookingDate);
    end.setHours(timeToHour(bookingEnd), 0, 0, 0);
  ...
  const hasDateTimeOverlap = (
    ...
    const booked = getBookingDateTimes(bookingDate, bookingStart, bookingEnd);
    const selected = getBookingDateTimes(selectedDate, selectedStart, selectedEnd);

    return selected.start < booked.end && selected.end > booked.start;
  };
  ...
  const handleCheck = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/check-availability?${params.toString()}`
      );
      ...
      setUnavailableCourts(data.unavailableCourts);
      setSelectedCourts([]);
      setShowCourts(true);
    } catch (error) {
      alert(
  ...
  const handleSubmitBookingRequest = async () => {
    ...
    try {
    if (!date || !startTime || !endTime || !selectedCourts || !customerName || !customerContact || !customerEmail) {
  ...
  const [paymentSubmitError, setPaymentSubmitError] =
    useState("");

  const [paymentSubmitSuccess, setPaymentSubmitSuccess] =
    useState("");
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
          ...
          <div className="bg-slate-900 rounded-3xl p-8 border border-slate-800">
            ...
            <div className="space-y-4">
              ...
              <div className="mt-5 space-y-3 border-t border-slate-700 pt-5 text-sm text-slate-300">
                ...
                <div>
                  <span className="font-medium text-slate-400">Contact Number: +639XXXXXXXXX</span>
                </div>

                <div>
                  <span className="font-medium text-slate-400">Contact Email: email@example.com</span>
                </div>
              </div>
            </div>
          </div>
                  ...
                  <span className="font-medium">
                    {receiptPreview ? (
                      ...
                      <div className="text-center">
                        ...
                        <p className="mt-1 text-xs text-slate-400">PDF selected</p>
                      </div>
                    ) : (
                      <div className="text-center">
                        <p className="text-sm font-medium">Attach payment receipt</p>
      ...
      {showBookingModal && (() => {
        ...
        const isOvernight =
          timeToHour(endTime) <= timeToHour(startTime);
        ...
        >
          ...
          >
            ...
            <div className="mb-5 rounded-xl bg-slate-800 p-4">
              ...
              <p>
                <span className="font-semibold">Booking Number: {bookingId} </span> (save this if you wish to pay later)
              </p>
            ...
            <div className="mt-6 flex gap-3">
              ...
              <button
                ...
                disabled={!customerName.trim() || !customerContact.trim() || isSubmitting}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-900 p-6 text-center shadow-2xl">
            <div className="mb-5 flex justify-start"> 
                <div className="text-center">
                  <p className="text-[11px]">Booking ID: {bookingId}
                  </p>
              </div>
            </div>
            ...
            <div className="mb-6 rounded-xl bg-slate-800 p-4 text-left">
              ...
              <p className="mt-2 text-sm text-slate-400">
                Please confirm your payment within 10 minutes to secure your slot. Use the "Submit Payment" form provided or send a screenshot of your 
                receipt with Booking Number as subject at basubasee@gmail.com.             
              </p>
              <p className="mt-5 text-sm text-slate-400 italic">Note: Bookings beyond 10pm will not be processed.</p>
            </div>
      ...
      <div className="border-b border-slate-700 p-4 sm:p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
          ...
          <button
            type="button"
            onClick={() =>
              setShowCalendarFilters((current) => !current)
            }
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    onClick={() => {
                      const allSelected =
                        selectedCalendarCourts.length === calendarCourts.length;
